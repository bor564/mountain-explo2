document.addEventListener("DOMContentLoaded", function () {
    const mountainDropdown = document.getElementById("mountainSelect");
 ...
 = locationsArray
.find(mountain => mountain.MountainName === selectedMountain);

        if (selectedlocationsArray
    ) {
            mountainInfoContainer.innerHTML = `<h2>${selectedlocationsArray
        .MountainName}</h2>
                                              <p><strong>Description:</strong> ${selectedlocationsArray
                                        .Description}</p>
                                              <p><strong>Elevation:</strong> ${selectedlocationsArray
                                        .Elevation} feet</p>`;
        }
    });
});
